...
namespace PretrageOsnovno
{
    class State
    {
        ...
        public List<State> children()
        {
            ...
            // ide kroz sve linkove
            foreach (Link link in this.Node.Links)
            {
                // pokupite endNode i cena linka link.EndNode link.Cost
                Node endNode = link.EndNode;
                double cenaLinka = link.Cost;
                // nextState(endNode, cena linka + this.Cost)
                // dodati u children listu
                children.Add(NextState(endNode, cenaLinka + this.Cost));
            }
