...
namespace Lavirint
{
    public class State
    {
        ...
        public bool dzip;
        public bool alarm;
        public bool dugme1zaGasenjeAlarma;
        public bool dugme2zaGasenjeAlarma;
        ...
        public State sledeceStanje(int markI, int markJ)
        {
            ...
            rez.dugme1zaGasenjeAlarma = this.dugme1zaGasenjeAlarma;
            rez.dugme2zaGasenjeAlarma = this.dugme2zaGasenjeAlarma;
            rez.mocvara = this.mocvara;
            rez.dzip = this.dzip;
            rez.alarm = this.alarm;
            return rez;
        }
        ...
        public List<State> mogucaSledecaStanja()
        {
            ...
            foreach (State senzor in Main.senzori)
            {
                double rastojanje = Math.Sqrt(Math.Pow(markI - senzor.markI, 2) + Math.Pow(markJ - senzor.markJ, 2));

                if (rastojanje < 2)  //NE PRILAZI senzoru
                {
                    Console.WriteLine("Alarm ukljucen");
                    alarm = true;
                }
            }

             //Dodajemo za PLAVU kutiju tj oruzje
            if (lavirint[markI, markJ] == 5){
                oruzje = true;
            }
            if (dugme1zaGasenjeAlarma && dugme2zaGasenjeAlarma)
            {
                alarm = false;
            }
            ...
            if ((mocvara || oruzje) && dzip == false && alarm!=true )
            { //Kretnja kao kralj u sahu
            ...
            else if(mocvara == false && oruzje == false && dzip == false && alarm != true)
            {//Kretnja kao lovac u sahu
            ...
            }else if(dzip == true&& alarm != true)
            {//Kretnja kao top i mocvarna polja ne vrede
        ...
        {
            int code = 10 * markI + markJ; //Ako nemamo oruzje ti ce nam biti od 0-200
            return oruzje ? code + 1000 : code; //Ako imamo oruzje taj kod nam ide dalje 
        }

        public bool isKrajnjeStanje()
        { //Proveravamo i da li smo pokupili kutiju to je uslov && kutija
            return Main.krajnjeStanje.markI == markI && Main.krajnjeStanje.markJ == markJ && alarm==false;
        }
